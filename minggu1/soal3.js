...
function digitsGrouper(numbers) {
  ...
  for(var x=0; x<numbers.length; x++){
    strNum.push(''+numbers[x])
  }
  for(var a=0; a<strNum.length;a++){
    if(strNum[a].length===1){
      satuDigit.push(strNum[a])
    }else if(strNum[a].length===2){
      duaDigit.push(strNum[a])
    }else if(strNum[a].length===3){
      tigaDigit.push(strNum[a])
    }else{
      empatDigit.push(strNum[a])
    }
  }
  var arrHasil =[]
